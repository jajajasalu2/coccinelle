...
struct_decl_one:
    ...
    | cv=ioption(const_vol) i=pure_ident_or_symbol d=direct_decl_option(disj_ident)
	 bf=struct_bitfield?
	 pv=TPtVirg
	 { let (id,fn) = d in
...
decl_var:
  ...
  | s=ioption(storage) t=ctype d=comma_list(direct_declarator(disj_ident)) pv=TPtVirg
      { List.map
	  (function (id,fn) ->
	    Ast0.wrap(Ast0.UnInit(s,fn t,id,[],P.clt2mcode ";" pv)))
	  d }
  | f=funproto { [f] }
  | s=ioption(storage) t=ctype d=direct_declarator(disj_ident) a=attr_list q=TEq e=initialize
      pv=TPtVirg
      {let (id,fn) = d in
      [Ast0.wrap
  ...
  | s=ioption(storage)
      ...
      { [Ast0.wrap
		...
		P.clt2mcode ";" p))] }
  | s=Ttypedef t=typedef_ctype d=comma_list(direct_declarator(typedef_ident)) pv=TPtVirg
...
one_decl_var:
    t=ctype pv=TPtVirg
      { Ast0.wrap(Ast0.TyDecl(t,P.clt2mcode ";" pv)) }
  | TMetaDecl { P.meta_decl $1 }
  | s=ioption(storage) t=ctype d=direct_declarator(disj_ident) a=attr_list pv=TPtVirg
      { let (id,fn) = d in
...
direct_declarator(ident_type):
  ...
  | d=direct_declarator(ident_type) o=TOPar arglist=decl_list(name_opt_decl) c=TCPar
      { let (id,fn) = d in
        (id, function t ->
              let r =
               Ast0.wrap
