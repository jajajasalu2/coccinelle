...
and field tgt decl =
  ...
  | Ast0.Field(ty,id,attr,bf,sem) ->
      let arity =
        match attr with
          [] -> all_same true tgt (mcode2line sem) [mcode2arity sem]
        | x::_ ->
            all_same true tgt (mcode2line x)
              (List.map mcode2arity attr @ [sem]) in
      let ty = typeC arity ty in
      ...
      let sem = mcode sem in
      make_field decl tgt arity (Ast0.Field(ty,id,attr,bf,sem))
