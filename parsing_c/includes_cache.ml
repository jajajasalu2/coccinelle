          ...
          (Ast_c.DeclList
            ([{Ast_c.v_namei = Some (n, _);
              ...
              let f _ =
                ...
                | Ast_c.NoSto
                | Ast_c.Sto _ ->
                    add_to_ret (RetVarOrFunc(s, (typ, Ast_c.NotLocalVar))) in
              f ()
      | Ast_c.Declaration
          (Ast_c.DeclList
            ([{Ast_c.v_namei = None;
              ...
              Ast_c.StructUnion (su, snameopt, def) ->
                (match snameopt with
                  Some s ->
                    ...
                    add_to_ret (RetStructUnionNameDef (s, ((su, def'),ii')))
                | None -> k p)
            | Ast_c.Enum(sopt, def) ->
...
let get_types_from_name_cache file name exp_type parse_fn =
    ...
    | Some (x, t) ->
        ...
        ignore
          (Visitor_c.vk_program
            (get_type_visitor file name env) ast);
        !env
