 ...
 * TODO: profile? *)
let get_type_from_includes_cache file name exp_type on_success on_failure =
  let file_bindings =
    Includes_cache.get_types_from_name_cache
      file name exp_type includes_parse_fn in
	  ...
	  then type_unfold_one_step t' env'
       ...
       with Not_found ->
          ...
          get_type_from_includes_cache
            f s Includes_cache.CacheTypedef
            (fun () ->
     ...
     | Ident (ident) ->
        ...
        | None  ->
            ...
            | None ->
                ...
                get_type_from_includes_cache
                  f s Includes_cache.CacheVarFunc
        ...
        | None  ->
            ...
            | None ->
                ...
                | None ->
                    ...
                    get_type_from_includes_cache
                      f s Includes_cache.CacheEnumConst
                ...
                let ret_typ =
                  (match (Ast_c.unwrap (snd t)) with
                    Ast_c.StructUnionName(su, sname) ->
                      get_type_from_includes_cache
                        f s (Includes_cache.CacheField sname)
                      (fun () ->
                        try
                          let ((su,fields),ii) =
